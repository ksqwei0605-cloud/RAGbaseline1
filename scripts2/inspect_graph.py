...
def load_pickle(path: Path) -> Any:
    """Load pickle file without importing heavy mmagent dependencies."""

    def build_placeholder_class(module: str, name: str) -> type:
        class Placeholder:
            ...
            def __repr__(self) -> str:
                return f"<Placeholder {module}.{name}>"
    ...
    class RobustUnpickler(pickle.Unpickler):
        """Custom unpickler that can handle unavailable project modules."""

        def find_class(self, module: str, name: str) -> Any:
            if module.startswith("mmagent"):
                return build_placeholder_class(module, name)
            try:
                return super().find_class(module, name)
            except (ImportError, ModuleNotFoundError):
                print(f"  [Note] Using placeholder for unavailable class: {module}.{name}")
                return build_placeholder_class(module, name)
...
def find_project_root(start_path: Path | None = None) -> Path:
    """Find the project root by looking for characteristic directories."""
    if start_path is None:
        start_path = Path(__file__).parent
    
    # Try to find project root by looking for mmagent and configs directories
    current = start_path.resolve()
    for _ in range(10):  # Search up to 10 levels
        # Check if this directory has the expected project structure
        if (current / "mmagent").exists() and (current / "configs").exists() and (current / "data").exists():
            return current
    ...
    # Fallback: assume we're in scripts, go up 2 levels
    fallback = Path(__file__).parent.parent.parent
    if (fallback / "mmagent").exists():
        return fallback
    
    raise RuntimeError(
        f"Could not find project root. Expected 'mmagent', 'configs', and 'data' directories. "
        f"Current detected root: {fallback}"
    )
